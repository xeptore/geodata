...
type geoipCategoryStats struct {
...
type geoipNormalizationStats struct {
	Invalid         int
	ExactDuplicates int
	CoveredNetworks int
}

// WriteGeoIP builds geoip.dat from configPath and returns the output path
// and the category list that was written.
func WriteGeoIP(configPath string) (string, *routercommon.GeoIPList, error) {
	config, err := loadGeoIPConfig(configPath)
	if err != nil {
		return "", nil, err
	}
	...
	for _, categoryConfig := range config.Categories {
		category, stats, err := buildGeoIPCategory(
			categoryConfig,
			cache,
		)
		if err != nil {
		...
		printGeoIPStats(category.GetCountryCode(), stats)
	}
	...
	return config.Output, output, nil
}

func loadGeoIPConfig(path string) (Config, error) {
	var config Config
...
func buildGeoIPCategory(
	config Category,
	cache map[string]*cachedGeoIPFile,
) (*routercommon.GeoIP, geoipCategoryStats, error) {
	var stats geoipCategoryStats
...
) ([]*routercommon.CIDR, geoipNormalizationStats) {
	var stats geoipNormalizationStats
...
func printGeoIPStats(name string, stats geoipCategoryStats) {
	fmt.Printf("%s:\n", name)
