...
type geositeCategoryStats struct {
	Collected        int
	ExactDuplicates  int
...
// WriteGeosite builds geosite.dat from configPath and returns the output path
// and the category list that was written.
func WriteGeosite(configPath string) (string, *routercommon.GeoSiteList, error) {
	config, err := loadGeositeConfig(configPath)
	if err != nil {
		return "", nil, err
	}
	...
	for _, outputConfig := range config.Categories {
		site, stats, err := buildGeositeCategory(outputConfig, cache)
		if err != nil {
			return "", nil, fmt.Errorf("build output category %q: %w", outputConfig.Name, err)
		}

		outputList.Entry = append(outputList.Entry, site)

		printGeositeCategoryStats(site.GetCountryCode(), stats)
	}
	...
	if err := writeGeoSiteDAT(config.Output, outputList); err != nil {
		return "", nil, err
	}

	if err := verifyGeoSiteDAT(config.Output); err != nil {
		return "", nil, fmt.Errorf("verify generated file: %w", err)
	}

	return config.Output, outputList, nil
}

func loadGeositeConfig(path string) (Config, error) {
	var config Config
...
func buildGeositeCategory(config Category, cache map[string]*cachedGeoSiteFile) (*routercommon.GeoSite, geositeCategoryStats, error) {
	var stats geositeCategoryStats
...
func printGeositeCategoryStats(
	name string,
	stats geositeCategoryStats,
) {
	fmt.Printf("%s:\n", name)
